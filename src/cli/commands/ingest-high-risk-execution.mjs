...
function parseIngestHighRiskExecutionOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--from") {
      ...
      if (!next || next.startsWith("--")) {
        return {
          ok: false,
          error: "nimicoding ingest-high-risk-execution refused: --from requires a value.\n",
        };
      }
    ...
    return {
      ok: false,
      error: `nimicoding ingest-high-risk-execution refused: unknown option ${arg}.\n`,
    };
  }
  ...
  if (!options.fromPath) {
    return {
      ok: false,
      error: "nimicoding ingest-high-risk-execution refused: explicit --from is required.\n",
    };
  }
